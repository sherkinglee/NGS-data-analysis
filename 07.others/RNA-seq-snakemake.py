...
LastEditTime: 2021-01-01 17:05:04
...
with os.popen("which htseq-count") as path:
    HTSEQCOUNT = path.read().strip()
...
rule all:
    input:
        ...
        expand("12.summary/RNA_seq_summary_statistics.txt")
...
rule Cutadapt:
    ...
    shell:
        """
        {CUTADAPT} -m 15 --match-read-wildcards -a {ADAPTER} -o {output} {input} > {log} 2>&1
        """
...
rule STAR:
    ...
    shell:
        ...
        {STAR} --runThreadN {threads} --outFilterType Normal --outWigType wiggle --outWigStrand Stranded \
            ...
            --outFilterMultimapNmax 20 --genomeDir {STAR_GENOME_INDEX} --readFilesIn {input} \
...
rule bam2bigwig:
    input:
        bam="07.STAR/{sample}_STAR/{sample}.Aligned.sortedByCoord.out.bam",
        bai="07.STAR/{sample}_STAR/{sample}.Aligned.sortedByCoord.out.bam.bai"
    output:
    ...
    shell:
        """
        {BAMCOVERAGE} -b {input.bam} -o {output} --normalizeUsing CPM --binSize 1 > {log} 2>&1
        """
...
rule htseq_count:
    ...
    shell:
        """
        {HTSEQCOUNT} -f bam -s yes -i gene_name -t exon -m intersection-strict {input} {GTFFILE} -c {output} > {log} 2>&1
        """
